...
const DEFAULT_APIS = [
  { slug: "phone",    name: "Phone Lookup",       url: "https://exploitsindia.site/api/number.php?exploits={query}",                       command: "/phone",    example: "9876543210",       pattern: "^[6-9]\\d{9}$",                                                               category: "Phone",    credits: 1, isActive: true  },
  { slug: "phone2",   name: "Phone Validator",    url: "https://api.bigdatacloud.net/data/phone-number-details?phoneNumber=%2B91{query}&key=", command: "/phone2", example: "9876543210",       pattern: "^[6-9]\\d{9}$",                                                               category: "Phone",    credits: 1, isActive: true  },
  { slug: "aadhaar",  name: "Aadhaar Lookup",     url: "https://exploitsindia.site/api/aadhar.php?exploits={query}",                       command: "/aadhaar",  example: "882838027159",     pattern: "^\\d{12}$",                                                                    category: "Identity", credits: 1, isActive: true  },
  { slug: "family",   name: "Family Info",        url: "https://exploitsindia.site/api/family.php?exploits={query}",                       command: "/family",   example: "882838027159",     pattern: "^\\d{12}$",                                                                    category: "Identity", credits: 1, isActive: true  },
  { slug: "vehicle",  name: "Vehicle RC",         url: "https://vehicle-info-api-abhi.vercel.app/?rc_number={query}",                     command: "/vehicle",  example: "KA04EQ4521",       pattern: "^([A-Z]{2}[0-9]{1,2}[A-Z]{0,3}[0-9]{1,4}|[0-9]{2}BH[0-9]{4}[A-Z]{1,2})$",  category: "Vehicle",  credits: 1, isActive: true  },
  { slug: "pan",      name: "PAN Card Lookup",    url: "https://pan.amorinthz.workers.dev/?key=AMORINTH&pan={query}",                     command: "/pan",      example: "ABCDE1234F",       pattern: "^[A-Z]{5}\\d{4}[A-Z]$",                                                       category: "Identity", credits: 1, isActive: true  },
  { slug: "ifsc",     name: "IFSC / Bank Info",   url: "https://ifsc.razorpay.com/{query}",                                               command: "/ifsc",     example: "SBIN0000001",      pattern: "^[A-Z]{4}0[A-Z0-9]{6}$",                                                      category: "Banking",  credits: 1, isActive: true  },
  { slug: "upi",      name: "UPI Lookup",         url: "https://api.b77bf911.workers.dev/upi?id={query}",                                 command: "/upi",      example: "user@fam",         pattern: "^[\\w\\.\\-]+@[\\w\\.\\-]+$",                                                  category: "Banking",  credits: 1, isActive: true  },
  { slug: "pincode",  name: "Pincode Info",       url: "https://api.postalpincode.in/pincode/{query}",                                    command: "/pincode",  example: "400001",           pattern: "^\\d{6}$",                                                                     category: "Location", credits: 1, isActive: true  },
  { slug: "ip",       name: "IP Lookup",          url: "https://ipapi.co/{query}/json/",                                                  command: "/ip",       example: "8.8.8.8",          pattern: "^(\\d{1,3}\\.){3}\\d{1,3}$",                                                  category: "Network",  credits: 1, isActive: true  },
  { slug: "email",    name: "Email Check",        url: "https://api.eva.pingutil.com/email?email={query}",                                command: "/email",    example: "test@gmail.com",   pattern: "^[\\w\\.\\-]+@[\\w\\.\\-]+\\.\\w+$",                                           category: "Email",    credits: 1, isActive: true  },
  { slug: "telegram", name: "Telegram User",      url: "https://exploitsindia.site/api/telegram.php?exploits={query}",                   command: "/telegram", example: "7459756974",       pattern: "^[\\d]{6,15}$",                                                                category: "Social",   credits: 1, isActive: true  },
  { slug: "pak",      name: "Pakistan Number",    url: "https://abbas-apis.vercel.app/api/pakistan?number={query}",                       command: "/pak",      example: "3001234567",       pattern: "^\\d{10,12}$",                                                                 category: "Phone",    credits: 1, isActive: true  },
  { slug: "global",   name: "Global Number",      url: "https://api.bigdatacloud.net/data/phone-number-details?phoneNumber={query}&key=", command: "/global",   example: "14155552671",      pattern: "^[\\+\\d][\\d]{7,15}$",                                                       category: "Phone",    credits: 1, isActive: true  },
  { slug: "ff",       name: "Free Fire Info",     url: "https://abbas-apis.vercel.app/api/ff-info?uid={query}",                          command: "/ff",       example: "123456789",        pattern: "^\\d{5,15}$",                                                                  category: "Gaming",   credits: 1, isActive: true  },
  { slug: "ffban",    name: "FF Ban Check",       url: "https://abbas-apis.vercel.app/api/ff-ban?uid={query}",                           command: "/ffban",    example: "123456789",        pattern: "^\\d{5,15}$",                                                                  category: "Gaming",   credits: 1, isActive: true  },
  { slug: "gstin",    name: "GSTIN Lookup",       url: "https://sheet.amorinthz.workers.dev/gst?gstin={query}",                          command: "/gstin",    example: "27AAPFU0939F1ZV",  pattern: "^\\d{2}[A-Z]{5}\\d{4}[A-Z]\\d[Z][\\dA-Z]$",                                  category: "Identity", credits: 1, isActive: true  },
  { slug: "domain",   name: "Domain WHOIS",       url: "https://api.hackertarget.com/whois/?q={query}",                                  command: "/domain",   example: "example.com",      pattern: null,                                                                           category: "Network",  credits: 1, isActive: true  },
];

// Always upsert ALL default APIs — handles both first run and new API additions
async function syncDefaultApis() {
  try {
    for (const api of DEFAULT_APIS) {
      await db.insert(osintApis).values({
        ...
        command: api.command,
        example: api.example,
        pattern: api.pattern ?? null,
        category: api.category,
        credits: api.credits,
        isActive: api.isActive,
      }).onConflictDoUpdate({
        target: osintApis.slug,
        set: {
    ...
    console.error("syncDefaultApis error:", err);
  }
}

syncDefaultApis().catch(console.error);
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = lib.get(url, {
      headers: {
        ...
        "User-Agent": "Mozilla/5.0 (compatible; CraKaOSINT/2.0)",
      },
    }, (res) => {
      let body = "";
      ...
      res.on("end", () => {
        ...
        try {
          ...
          // Handle top-level arrays — flatten first element or wrap
          if (Array.isArray(parsed)) {
            if (parsed.length === 0) {
              resolve({ data: { result: "No data found" }, statusCode });
            } else if (parsed.length === 1 && typeof parsed[0] === "object" && parsed[0] !== null) {
              // Single-item array — unwrap the object
              resolve({ data: parsed[0] as Record<string, unknown>, statusCode });
            } else {
              // Multi-item array — nest under 'results'
              const merged: Record<string, unknown> = {};
              parsed.forEach((item, i) => {
                if (typeof item === "object" && item !== null) {
                  const obj = item as Record<string, unknown>;
                  for (const [k, v] of Object.entries(obj)) {
                    merged[i === 0 ? k : `${k}_${i + 1}`] = v;
                  }
                } else {
                  merged[`item_${i + 1}`] = item;
                }
              });
              resolve({ data: merged, statusCode });
            }
          } else if (parsed && typeof parsed === "object") {
            resolve({ data: parsed as Record<string, unknown>, statusCode });
          } else {
            resolve({ data: { result: String(parsed) }, statusCode });
          }
        } catch {
          // Non-JSON response (e.g. WHOIS plaintext)
...
// Detect a genuinely failed API response
function isApiError(data: Record<string, unknown>, statusCode: number): boolean {
  if (statusCode >= 400) return true;
  if (!data || Object.keys(data).length === 0) return true;

  // Explicit error indicators
  const hasErrorMsg = "error" in data && data.error && typeof data.error === "string" && data.error.length > 0 && Object.keys(data).length <= 3;
  const hasStatusFalse = "status" in data && data.status === false;
  const hasSuccessFalse = "success" in data && data.success === false && Object.keys(data).length <= 3;

  // API-specific: only a 'raw' key with empty content
  const onlyRawEmpty = Object.keys(data).length === 1 && "raw" in data && String(data.raw).trim().length === 0;

  return hasErrorMsg || hasStatusFalse || hasSuccessFalse || onlyRawEmpty;
}
...
router.post("/osint/lookup", async (req, res) => {
  ...
  // Token validation
  let user: any = null;
  const userSessionId: string | null = sessionId ?? null;
  ...
  if (!user) {
    res.status(401).json({ error: "Session not found. Please refresh and try again." });
    return;
  }
  ...
  if (!isUnlimited) {
    if (user.creditsEarned < apiRow.credits) {
  ...
  try {
    ...
    if (isApiError(rawData, statusCode)) {
...
router.delete("/osint/history/all", async (req, res) => {
  ...
  try {
    await db.delete(osintHistory).where(eq(osintHistory.sessionId, sessionId));
...
router.delete("/osint/history/:id", async (req, res) => {
  ...
  try {
    ...
    if (!entry[0]) { res.status(404).json({ error: "Log entry not found or does not belong to you" }); return; }
